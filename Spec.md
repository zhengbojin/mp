...
29. **判定域与时间语义、结论强度定位**：
    ...
    - **结论强度定位（不可误读）**：
      ...
      - **经典定义的问题与改进（纲领）**：经典 P/NP 定义在表示层与语义层存在缺陷——**P 侧**：sep/# 与数据不可区分、边界先验（违背信息论）、blank 双职；**NP 侧**：分支同一性悖论、见证本体论错位、对偶性被量词消除——**定义必须改进**。改进 = **符号图灵机**（结构/边界/分支语义内建，|Σ| ≥ 4，输入 = 元素向量）。**只有在改进为符号图灵机之后，算法复杂性困难的本质原因才被真正获得：本质原因 = 维度 κ**（0 vs Ω(n)；证明全链不含时间变量）。
      - **一般确定性机的对照（经典 P 类的实质内容）**：**存在一般确定性机在指数时间内解出子集和**（枚举所有选择组合并逐一验证）（注：『一般确定性机』的『求解』不构成良定义对象——求解须在纸带上区分 sep 与边界符，而不受字母表限制时输入失去定义；该对照句仅在名义/对照意义上使用）——故对一般确定性机，「解不了」**只在多项式时间约束下成立**；「一般确定性机多项式时间不可解」（= 经典 P ≠ NP 的实质）**不在本工程已证范围**（**两层理由，防误读**：①**对象层**——『求解』不构成良定义对象（见本条前注）⇒ 该句先在对象层退场：**不是「未证」，而是不可作为工程命题**；②**证明力层**——即便按名义读法取其「经典 P ≠ NP 的实质」义，本工程证明力亦只在受限机类（能力缺口），**小类强结论不蕴含大类命题**，且一般机侧唯一可能的支撑（**时间下界**）未证。⇒ 该句：**框架纪律下 = 对象不成立；名义读法下 = 未证且弱于**）。相应地：**受限机类 ⊆ 一般确定性机类 ⇒ `P_sym0` ⊆ 经典 P 类 ⇒ `P_sym0 ≠ NP_sym0` 不蕴含经典 `P ≠ NP`**。
      - **多项式条件的地位**：在类定义层限定「考察哪一类」；在**受限机侧不承担证明负荷**（能力缺口），在**一般确定性机侧**（注：『求解』不构成良定义对象——见上条）**才是实质条件**（指数时间上界的存在 ⇒ 时间下界的论证必要且未证）。
      - **「多项式时间」的三层分工（注）**：① 定义层——验证器类 `Verifiers`/`FVerifiers` 以多项式时间为**成员条件**（κ(L) 的最小化在多项式验证器类内进行）；② 构造侧——验证机时间界**已证** O(κ·|x|²)（κ = 元素个数；|x| 可退化为布尔长度）；③ 分离/κ 下界证明——不引用该条件（受限机的不可能性在任何时间预算下成立）。
...
31. **符号层复杂度类与经典分离结论（P_sym0 ≠ NP_sym0）**：
    ...
    - **分离结论**：**`P_sym0 ≠ NP_sym0`**（集合论意义：两个语言类的集合不等）；见证语言 = 子集和（`subsetSumLanguageF4Real ∈ NP_sym0` 且 `∉ P_sym0`）。精确读法：**受限字母表机器类（多项式时间）≠ NTM2 类（多项式时间）**——P 侧证据为能力缺口（与时间界无关），且**弱于经典 P ≠ NP**（受限机类 ⊆ 一般确定性机类；**注：『一般确定性机』的『求解』不构成良定义对象**，见约定 29；信息论前提见约定 29）。**与 F 层 `P_F ≠ NP_F` 是同一定理的两种表述**（定义级相等；定位见约定 29、解释见 §7.7）。
...
- **时间条件的正确归属（不可误读）**：上述否定**与时间界无关**（证明未引用 `_hpoly`）——但**这一点只对受限机成立**。**存在一般确定性机在指数时间内解出子集和**（枚举 + 验证）（注：『一般确定性机』的『求解』不构成良定义对象——见约定 29），故对一般确定性机，「解不了」**只在多项式时间约束下成立**；「一般确定性机多项式时间不可解」（经典 P≠NP 的实质）**不在本工程已证范围**（**两层理由·防误读**见约定 29）。
- **与经典 P≠NP 的距离**：**受限机类 ⊆ 一般确定性机类 ⇒ `P_sym0` ⊆ 经典 P 类** ⇒ 本结论（受限机类 ≠ NTM2 类）**弱于**经典 P ≠ NP（**注：『一般确定性机』的『求解』不构成良定义对象**——求解须在纸带上区分 sep 与边界符，而不受字母表限制时输入失去定义；故包含关系与『弱于』对照仅在名义意义上使用）。加之两层外部语义：**结构语义**（引约定 20：经典 P 类定义中的机器只读符号串，sep/# 与数据不可区分——结构身份为外部命名，可逆标签；区分结构需要表示能力，|Σ| ≥ 4 最小完备）；**边界语义**（引约定 23：「输入在何处结束」为外部给定——δ 无输入结束符号/状态，blank 机制依赖「输入内无 blank」的隐含承诺，单看纸带+无限比特流则边界不可判定；**该先验假设违背信息论**：边界为非零信息量而纸带与语法均不携带，定义外部注入 ⇒ 语义不闭合）。故「经典 P 类 ↔ 符号层类」的对比**必须经编码桥**，桥须论证结构与边界**两种语义**的对应，其合法性/信息损失即本结论「框架内」限定的技术内容。
