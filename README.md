...
> **版本 v2.4**（交付包 `mp.ver2.4.zip`；版本谱系：v0.1.0 → v1.0 → v2.0 → v2.1 → v2.2 → v2.3 → v2.4）
...
**与经典 P≠NP 的距离**：本结论**弱于**经典 P ≠ NP——受限机类 ⊆ 一般确定性机类（`P_sym0` ⊆ 经典 P 类）；且经典 P 类定义中的机器**只读符号串，sep/# 与数据不可区分**（结构身份为外部命名；区分结构需表示能力，|Σ| ≥ 4——信息论，Spec 约定 20），故与经典层的对比须经编码桥。另注：**『一般确定性机』的『求解』不构成良定义对象**——求解须在纸带上区分 sep 与边界符，而不受字母表限制时输入失去定义；其『指数时间解』『多项式不可解』诸断言仅在名义/对照意义上使用；该口径下的分离不在本工程已证范围（**两层防误读**：①对象层——『求解』无定义 ⇒ 不可作为工程命题；②名义层——未证且弱于）。
